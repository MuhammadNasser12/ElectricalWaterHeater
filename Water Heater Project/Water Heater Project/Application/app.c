...
u8 state = OFF;  		//off state by default
...
static f32 TempSense(void)
{
	...
	{
		/* timer calculation
		 * required time = (# of ticks * tick time)
		 * 100 ms 		 =  		   * (1/(1^6/256))
		 * # of ticks 	 = 		390.625
		 * # of overflows= # of ticks / resolution
		 * # of overflows=  390.625 / 256 = 1.5258
		 * then we will use 2 overflows but the 1st one will not start from 0
		 * initial value:
		 * 		(2-1.5258)*256 = 121.39
		*/
...
static void setState(void)
{
	...
	while(1)
	{
		...
		else
		{
			...
			/*timer calculation
			 * required time = # of ticks * tick time
			 * 1000 ms 	 	 =      x     * ( 1 /(1*10^6/1024))
			 * # of ticks 	 =  	976.5 tick
			 * # of overflows=   # of ticks / Resolution
			 * 				 =		976.5 / 256  = 3.8146
			 * then we will need 4 overflows but the 1st one will not start from zero
			 * # of ticks required from 1st overflow 0.8146 *256 = 208
			 * initial value = 256 - 208 = 48
